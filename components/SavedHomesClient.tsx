...
import { FINQIT_STATE_EVENT, getSavedHomes } from "@/lib/demo-state";

export function SavedHomesClient() {
  const [saved, setSaved] = useState<string[]>([]);

  useEffect(() => {
    const sync = () => setSaved(getSavedHomes());
    sync();
  ...
  return <div className="saved-page">
    ...
    {homes.length ? <>
      <div className="saved-count"><HeartIcon size={16} fill="currentColor"/><strong>{homes.length}</strong> saved {homes.length === 1 ? "home" : "homes"}</div>
      <div className="property-grid">{homes.map((property) => <PropertyCard key={property.slug} property={property}/>)}</div>
